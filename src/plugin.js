...
(function() {
  tinymce.PluginManager.add('variable', function (editor) {
     ...
     * @type {object}
     */
    var mapper = (editor, value) => editor.variables.find(variable => variable.key === value)?.displayname || value;
    ...
    AutoComplete.prototype = {
      ...
      renderInput: function () {
        var rawHtml = '<span id="autocomplete-variable">' +
      ...
      lookup: function () {
        ...
        if (this.query === '') {
          var autoComplete = body.find('#autocomplete-variable');
      ...
      select: function (item) {
        this.editor.focus();
        $(this.editor.dom.select('span#autocomplete-variable')).replaceWith(this.insert(item));
        stringToHTML();
      ...
      cleanUp: function (rollback, complete) {
        ...
        if (rollback) {
          var text = this.query,
              $selection = $(this.editor.dom.select('span#autocomplete-variable'));
      ...
      offset: function () {
        var rtePosition = $(this.editor.getContainer()).offset(),
            ...
            nodePosition = $(this.editor.dom.select('span#autocomplete-variable')).position(),
      ...
      offsetInline: function () {
        var nodePosition = $(this.editor.dom.select('span#autocomplete-variable')).offset();
    ...
    editor.addVariable = addVariable;
    editor.setVariables = setVariables;
    // expose variables, so it can be used for the sidepanel
    editor.variables = autoCompleteData.source;
    editor.$ = $;

    editor.ui.registry.addToggleButton('variable', {
      icon: 'settings',
      tooltip: 'Variablen anzeigen/ausblenden',
      onSetup: (api) => {
        api.setActive(!api.isActive());
        setTimeout(() => editor.execCommand('ToggleSidebar', true, 'variablesidebar'), 500);
      },
      onAction: (api) => {
        editor.execCommand('ToggleSidebar', false, 'variablesidebar');
        api.setActive(!api.isActive());
      },
    });

    editor.ui.registry.addSidebar('variablesidebar', {
      onShow: (api) => {
        const variables = editor.variables;
        const content = [
          '<div class="mce-menu mce-menu-variables">',
          '<input class="mce-input-search" placeholder="Suche" />',
        ...
        variables.forEach(
            variable => {
              content.push(
                  `<div class="mce-menu-item" tabindex="-1" data-variable="${ variable.key }">`,
                  `<span class="mce-text">${ variable.displayname }</span>`,
                  `</div>`,
              );
            },
        );
        ...
        api.element().innerHTML = content.join('\n');
        $('.mce-menu-item').on('click', function() {
          editor.addVariable($(this).attr('data-variable'));
        });
        $('.mce-input-search').on('input', function() {
          const value = this.value;
          $('.mce-menu-item').each(
              function(index, el) {
                if (!value) {
                  $(el).fadeIn();
                } else if ($(el).children().text().toLowerCase().indexOf(value.toLowerCase()) === -1) {
                  $(el).fadeOut();
                } else {
                  $(el).fadeIn();
                }
